 ...
 */
export function mapEcsTaskToDispatchStatus(task: EcsTaskDetail): StatusMappingResult {
 ...
 */
export function extractDispatchId(task: EcsTaskDetail): string | undefined {
  ...
  if (task.overrides?.containerOverrides) {
    for (const containerOverride of task.overrides.containerOverrides) {
      if (containerOverride.environment) {
        const dispatchEnv = containerOverride.environment.find(
          env => env.name === 'DISPATCH_ID'
        );
  ...
  // 3. Check task tags
  if (task.tags) {
    const dispatchTag = task.tags.find(
      tag => tag.key === 'dispatch_id' || tag.key === 'dispatchId' || tag.key === 'DISPATCH_ID'
    );
 ...
 */
export function shouldProcessTask(task: EcsTaskDetail): { process: boolean; reason?: string } {
